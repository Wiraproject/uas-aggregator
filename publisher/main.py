...
DELAY = float(os.getenv("DELAY", "0.005")) # Dipercepat agar 20rb event tidak terlalu lama
DUPLICATION_RATE = float(os.getenv("DUPLICATION_RATE", "0.3")) 
MAX_EVENTS = int(os.getenv("MAX_EVENTS", "20000")) # Batas stop otomatis

sent_events_history = []
MAX_HISTORY = 2000 # Diperbesar sedikit history-nya
...
def run_publisher():
    ...
    while count < MAX_EVENTS:
        try:
            ...
            # Print status setiap 500 request agar log tidak banjir
            if count % 500 == 0:
                print(f"Progress: {count}/{MAX_EVENTS} events sent... (Last: {response.status_code})")
            
            count += 1
            
            # Delay kecil (bisa diatur 0 di docker-compose jika ingin ngebut pol)
            if DELAY > 0:
                time.sleep(DELAY)
    ...
    # Biarkan container tetap hidup agar bisa dicek log-nya, jangan langsung exit
